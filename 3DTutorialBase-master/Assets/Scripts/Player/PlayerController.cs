...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("A list of all attacks information")]
    ...
    // current moving direction of the player
    private Vector2 p_Velocity;

    // at the very beginning we cannot be frozen; timer set to 0
    private float p_FrozenTimer;

    // the default color; cached here
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < m_Attacks.Length; i++)
        {
            ...
            {
                Debug.LogError(attack.AttackName + " has a wind up time larger than the amount of time the player if frozen for. ");
            }
        }
    }
